...
- [x] Venue/location
  ...
  - **Geocoder limitation:** `/map/ajax` only resolves coarse place names — venue names and street addresses usually return `value: null` (e.g. "1214 Main Street Hartford Connecticut" fails, "Hartford, Connecticut" works). `resolveVenueGeo` walks full search → address+city → special-template `venue.geo` (exact coordinates, preferred over a city-level pin) → city.
...
- [x] Special event templates (`templates/<keyword>/template.json`)
  ...
  - A special template can also carry `venue.geo` (exact coordinates used when the map search can't resolve the address — see the geocoder limitation under Venue/location). The baseball template pins Dunkin' Park.
  - Verified live on draft 188278 (evt-2026-008): full `populate` end-to-end — static blocks + substituted schedule, blueprint pin, banner, venue with template coordinates, ticket pricing (Members complimentary / Public $15), and both campaigns.
  - Ticket findings from that run: the edit-price modal's price-type codes are `Online` (Standard price), `Atdoor` (Door price), `Free` (Complimentary) — there is no `Standard` code, and sending one gets a 500. The modal doesn't dismiss on Escape; its visible `a.cancel-button` is the reliable close (the `StandardForm::hide` "Close" anchor is invisible).
  - Campaign endpoints validate the campaigns-page token, not the TOKEN-cookie CSRF an env override carries — the campaign helpers now always fetch a fresh `/campaigns/list/` token and only fall back to the provided one.
  - Banner upload verified live too (draft 188278 header now shows the Dunkin' Park banner). Note for future debugging: `/upload/images` went through a ~day-long stretch of returning app-level `{code:504, "Sorry, an error occurred", duration "0ms"}` for **every** upload — including byte-identical replays of previously-working requests and Glue Up's own design-customizer UI driven in a browser — then recovered with no client-side change (the same 960×480 JPEG that failed uploaded fine afterwards, with the 1000/1920 variants generated). Diagnosis: transient failure in Glue Up's upload/image-processing backend. Ruled out with controlled tests: payload shape, token type (fresh page-embedded token was otherwise valid), image size/format, and session state (authenticated GETs and `publicPageSubmit` succeeded in the same session while uploads failed). If this recurs, don't rewrite the upload code — retry later.
  - Separate flakiness: the headless saved-session probe (`getGlueUpSession`) can fail with `GLUEUP_LOGIN_REQUIRED` when Cloudflare challenges the headless page load even though the cookies are still valid — raw `fetch` calls with the profile's cookie header keep working. Env `GLUEUP_COOKIE`/`GLUEUP_CSRF_TOKEN` override is the workaround.
