...
- [~] Event banner image (in progress — paused after Stage 1)
  ...
  - Decisions: rank candidates by **AI vision** (OpenAI, `detail: low`); **convert HEIC→JPEG** in CI via a `heic-convert` dependency.
  ...
  - Stages 2–4 DONE: `selectBannerCandidate` (`src/generate/bannerSelector.js`) downloads the candidates' upsized thumbnails (`=s800`) and ranks them with the `/v1/responses` vision API (`config.openaiModel`, `detail:"low"`, strict-JSON `chosenId`+`ranking`). `prepareBannerImage` (in `prepare`) downloads only the winner's original, converts via `toBannerImage` (HEIC→JPEG with lazily-imported `heic-convert`; PNG kept; else `.jpg`), and writes `banner.jpg` (or `.png`) + `banner.json` (sourceId/name/folder/reason/ranking) into the run. Falls back to the newest candidate when ranking is unavailable (no key / no thumbnails) so a banner is still produced. All non-fatal — failure logs and continues.
  ...
  - Verified locally: `npm run check`, `npm ci`, HEIC→JPEG conversion, and the selector's guard paths. NOT yet run end-to-end in CI (needs Drive + OPENAI creds) — confirm the next monthly-prepare produces `banner.jpg`/`banner.json`, then `populate` uploads it automatically.
