...
- [~] Event banner image (in progress — paused after Stage 1)
  ...
  - Verified locally: `npm run check`, `npm ci`, HEIC→JPEG conversion, and the selector's guard/fallback paths. NOT yet run end-to-end (no local Drive/Gemini creds) — confirm the next monthly-prepare produces `banner.jpg`/`banner.json`. **CI risk to watch:** the Gemini call 403s if the service account `sheets@gen-lang-client-0848431620…` lacks access to a project with the Generative Language API enabled. If so, grant the SA `roles/serviceusage.serviceUsageConsumer` (or use Vertex AI) — but the fallback keeps prepare working regardless.
