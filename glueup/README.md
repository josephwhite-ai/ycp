...
Special event templates live under `templates/<keyword>/template.json`. If `<keyword>` or any configured keyword appears in the summary sheet, `prepare` records `manifest.specialTemplate`. Local `ensure` resolves that special template from its configured source page, and `populate-page` prefers it over any live/prose Glue Up page-template reference. The first special template is `templates/baseball`, based on the Hartford Yard Goats page.
