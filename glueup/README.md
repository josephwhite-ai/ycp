...
If the event summary sheet includes a Glue Up event URL in prose, `prepare` records it in `manifest.json` as a deferred Glue Up reference rather than trying to read Glue Up from GitHub Actions. During local `ensure`, after the authenticated session is available, page-template references are resolved from the source event's Website > Design page and stored back into the manifest. `populate-page` uses the resolved page template when present, clearing source block IDs so the target draft receives fresh Glue Up IDs.

Special event templates live under `templates/<keyword>/template.json`. If `<keyword>` or any configured keyword appears in the summary sheet, `prepare` records `manifest.specialTemplate`. A special template can carry: static page blocks (`pageTemplate.content`, with `{{scheduleHtml}}` substituted per event — resolved immediately, no Glue Up read), a pinned blueprint (`glueUp.eventType`/`blueprintCode`, applied to `template-selection.json` so event types outside the approved taxonomy still select a blueprint), and a banner (`banner.sourceUrl`, downloaded into the run dir during local `ensure` and pushed by `populate-banner`; a manually dropped banner is left alone). A template with only a `pageTemplate.sourceUrl` is instead resolved live from that event's Website > Design page during `ensure` — this only works for events built on the current v2 site templates. `populate-page` prefers the special template over any prose Glue Up page-template reference, and refuses to run while a matched special template is unresolved. The first special template is `templates/baseball`: static blocks mirroring the Hartford Yard Goats page (145378, which predates the v2 templates and cannot be resolved live), its Dunkin' Park banner, and the public NHH blueprint `90663`.

If the event summary references payment, ticket prices, complimentary member tickets, or member/non-member discounts, `prepare` records `manifest.ticketing`. Local `ensure` backfills this for older artifacts. `populate` configures Glue Up's ticket/payment setup from that manifest, currently targeting the approved two-ticket template shape: `Members` and `Public` ticket rows with member-only and public price options.
